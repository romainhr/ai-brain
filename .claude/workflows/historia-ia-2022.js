export const meta = {
  ...
  description: 'Historia de la IA (nov 2022 -> hoy) como notas evergreen en vault/40_Foundations, con routing multi-motor para estirar tokens',
  phases: [
    { title: 'Investigar', detail: 'research web por eje en Gemini CLI (coste cero) con fallback a Claude sonnet' },
    { title: 'Redactar', detail: 'una nota foundation por eje en Claude haiku (datos verificados -> markdown)' },
    { title: 'Verificar', detail: 'QA mecánico por nota en Claude haiku (fechas/URLs/tags/wikilinks)' },
    { title: 'Tejer', detail: 'índice maestro + _Home-Foundations en Claude sonnet' },
  ],
}
...
const QA_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  required: ['slug', 'ok', 'problemas'],
  properties: {
    slug: { type: 'string' },
    ok: { type: 'boolean', description: 'true si la nota pasa todos los checks' },
    n_hitos: { type: 'integer' },
    problemas: {
      type: 'array',
      items: { type: 'string' },
      description: 'lista de problemas concretos: fechas dudosas, URLs sospechosas, tags fuera de taxonomía, wikilinks a notas inexistentes, formato',
    },
  },
}
...
// Encargo de research (autocontenido; lo ejecuta Gemini vía puente, o Claude en fallback).
const researchEncargo = (eje) =>
  ...
  `Verifica fechas de lanzamiento y URLs de fuentes primarias (blogs oficiales, papers de arXiv, releases) con búsqueda web. ` +
  ...
  `Devuelve EXCLUSIVAMENTE un JSON con esta forma: {"slug","title","hitos":[{"nombre","fecha","quien","que_cambio","url"}],"related_sugeridos":[],"notas_fuera_de_taxonomia":[]}. Mínimo 8 hitos (idealmente 12-25).`

// Puente a Gemini CLI vía engine-runner (lee pipeline/model-routing.yaml). Coste cero (suscripción Google).
const geminiBridge = (encargo) =>
  `ENGINE: gemini\n` +
  `Lee pipeline/model-routing.yaml -> engines.gemini para obtener 'invoke' (prefijo de comando) y 'model'. Si 'model' está vacío, NO pases -m.\n` +
  `MODEL: (vacío)\n` +
  `Sigue tu procedimiento: vuelca el ENCARGO a un archivo temporal, ejecuta el CLI con timeout, captura stderr APARTE (nunca 2>&1; NO uses -o json: cuelga), extrae el JSON del stdout (recorta a las llaves) y valida contra el esquema.\n\n` +
  `ENCARGO (pásalo por stdin al CLI):\n${encargo}`

phase('Investigar')
log(`Documentando la historia de la IA (${PERIODO}) en ${EJES.length} ejes. Research -> Gemini CLI (coste cero) con fallback Claude sonnet.`)

const resultados = await pipeline(
  EJES,
  // Stage 1: research verificado — Gemini CLI (coste cero) con fallback a Claude sonnet.
  async (eje) => {
    let r = await agent(geminiBridge(researchEncargo(eje)), {
      label: `research:${eje.slug} [gemini]`, phase: 'Investigar', schema: RESEARCH_SCHEMA, agentType: 'engine-runner',
    })
    if (!r || !Array.isArray(r.hitos) || r.hitos.length < 8) {
      log(`research ${eje.slug}: Gemini insuficiente -> fallback Claude sonnet`)
      r = await agent(
        researchEncargo(eje) + `\n\nUsa WebSearch y WebFetch para verificar fechas y URLs reales.`,
        { label: `research:${eje.slug} [fallback sonnet]`, phase: 'Investigar', schema: RESEARCH_SCHEMA, model: 'sonnet' }
      )
    }
    return r ? { ...r, _slug: eje.slug } : null
  },
  // Stage 2: redacción de la nota foundation — Claude haiku (datos verificados -> markdown).
  (research, eje) => {
    ...
    return agent(
      ...
      { label: `write:${eje.slug}`, phase: 'Redactar', schema: WRITE_SCHEMA, model: 'haiku' }
    )
  },
  // Stage 3: QA mecánico — Claude haiku (verifica fechas/URLs/tags/wikilinks).
  (writeRes, eje) => {
    if (!writeRes) return null
    return agent(
      `Verifica la nota ${FND}/${eje.slug}.md (usa Read). Comprueba de forma mecánica:\n` +
      `- Frontmatter foundation/evergreen correcto y tags SOLO de pipeline/taxonomy.yaml (más 'history'); léela.\n` +
      `- Cada wikilink en 'related' apunta a una nota que EXISTE (revisa vault/20_Items/ y vault/40_Foundations/).\n` +
      `- Fechas con formato YYYY-MM o YYYY-MM-DD; señala fechas implausibles o futuras (> 2026-06).\n` +
      `- URLs con forma válida (http/https); señala las claramente inventadas o vacías.\n` +
      `- Cuenta los hitos de la tabla cronológica.\n` +
      `Devuelve el JSON del esquema. NO edites la nota; solo reporta problemas concretos.`,
      { label: `qa:${eje.slug}`, phase: 'Verificar', schema: QA_SCHEMA, model: 'haiku' }
    ).then(qa => ({ ...writeRes, qa }))
  }
)

const notas = resultados.filter(Boolean)
const conProblemas = notas.filter(n => n.qa && n.qa.ok === false)
log(`${notas.length}/${EJES.length} notas escritas. QA con problemas: ${conProblemas.length}. Tejiendo índice maestro.`)
...
const indice = await agent(
  ...
  { label: 'indice-maestro', phase: 'Tejer', model: 'sonnet' }
)

return {
  notas_eje: notas.map(n => ({ slug: n.slug, n_hitos: n.n_hitos, path: n.path, qa_ok: n.qa ? n.qa.ok : null })),
  total_notas: notas.length,
  notas_con_problemas: conProblemas.map(n => ({ slug: n.slug, problemas: n.qa.problemas })),
  indice,
}
